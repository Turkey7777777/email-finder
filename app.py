...
# Autosave paths (/tmp is writable on Streamlit Cloud)
...
# Track domains finished (even if 0 emails)
...
async def crawl_domain(client: httpx.AsyncClient, base_url: str, max_pages=PER_DOMAIN_PAGE_LIMIT) -> set:
    ...
    # Seed likely people/contact pages
    for path in [
...
# Fallback controls: prefer people; only use role aliases when NO names are found
enable_fallback = st.checkbox("Enable fallback guessing for people (when no evidence)", value=True)
...
if file_buf:
    ...
    # ===== DOMAIN CRAWL MODE =====
        ...
        async def run_crawl():
            async with httpx.AsyncClient(follow_redirects=True, headers={"User-Agent": USER_AGENT}) as client:
                for i, base in enumerate(bases, start=1):
                    ...
                    # ---- On-the-fly pattern inference + generation per domain (people-first) ----
                    people_paths = [
                        "/about", "/about-us", "/team", "/our-team", "/leadership", "/management",
                        "/company", "/staff", "/providers", "/our-providers", "/doctors", "/our-doctors",
                        "/meet-the-team", "/who-we-are", "/news", "/blog", "/contact", "/contact-us"
                    ...
                    if pattern and conf >= 0.34 and uniq_names:
                        ...
                        # not enough evidence -> people-first fallback guesses
                        ...
                        # Only when NO names exist, optionally add generic role aliases (up to 7)
        ...
        # Show generated contacts (people-first; role aliases only if no names)
